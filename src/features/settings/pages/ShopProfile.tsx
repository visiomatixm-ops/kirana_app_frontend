...
export default function ShopProfile({ onClose, activeScreen, onNavigate }: ShopProfileProps) {
  ...
  const { status: gstStatus, message: gstMessage } = useGstVerify(formData.gstin);
            ...
            <div className="space-y-4">
              ...
              <div>
                <input
                  ...
                  className="w-full bg-white border border-[#DBDBDB] rounded-xl px-4 py-2.5 text-sm text-[#223960] placeholder:text-[#CAC7C7] outline-none transition-colors"
                />
                {gstMessage && (
                  <span
                    style={{
